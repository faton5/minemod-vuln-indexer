...
items = []
for kind in ("repositories", "vulnerabilities", "findings", "provider_project_groups"):
    for row in queries.load_records(database, kind):
        ...
        if row.get("requires_manual_review") or unresolved_repository or provider_conflict:
